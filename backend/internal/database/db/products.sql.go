...
RETURNING id, category_id, title, description, price, status, created_by, created_at, updated_at, commission_amount, location_name, latitude, longitude, province, regency, district, village, stock, member_commission_amount, reseller_commission_amount, specifications
`
...
func (q *Queries) CreateProduct(ctx context.Context, arg CreateProductParams) (Product, error) {
	row := q.db.QueryRowContext(ctx, createProduct,
		...
		arg.Description,
		arg.Price,
		arg.MemberCommissionAmount,
		arg.ResellerCommissionAmount,
...
SELECT id, category_id, title, description, price, status, created_by, created_at, updated_at, commission_amount, location_name, latitude, longitude, province, regency, district, village, stock, member_commission_amount, reseller_commission_amount, specifications FROM products
WHERE id = $1 LIMIT 1
`
...
const getProductAsset = `-- name: GetProductAsset :one
SELECT id, product_id, asset_type, object_key, created_at FROM product_assets WHERE id = $1 LIMIT 1
`

func (q *Queries) GetProductAsset(ctx context.Context, id uuid.UUID) (ProductAsset, error) {
	row := q.db.QueryRowContext(ctx, getProductAsset, id)
	var i ProductAsset
	err := row.Scan(
		&i.ID,
		&i.ProductID,
...
SELECT p.id, p.category_id, p.title, p.description, p.price, p.status, p.created_by, p.created_at, p.updated_at, p.commission_amount, p.location_name, p.latitude, p.longitude, p.province, p.regency, p.district, p.village, p.stock, p.member_commission_amount, p.reseller_commission_amount, p.specifications, 
